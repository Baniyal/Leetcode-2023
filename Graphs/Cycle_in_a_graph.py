...
Find all the vertices which are not visited and are adjacent to the current node. Recursively call the function for
...
def is_dag_cyclic(n, edges):
    ...
    def is_dag_cyclic_utility(curr_node,path):
        ...
        for neigh_node in graph[curr_node]:
            print("Enterning-------->")
            print("neighbor--------->", neigh_node)
            if neigh_node not in path :
...
print(is_dag_cyclic(N, RELATIONS))